//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "QKBaseViewController.h"

#import "UIScrollViewDelegate-Protocol.h"

@class HMSegmentedControl, NSMutableDictionary, NSString, UIScrollView;

@interface QKCollectViewController : QKBaseViewController <UIScrollViewDelegate>
{
    HMSegmentedControl *_segmentControl;
    UIScrollView *_contentScroll;
    NSMutableDictionary *_controllers;
    NSString *_collectionType;
}

@property(copy, nonatomic) NSString *collectionType; // @synthesize collectionType=_collectionType;
@property(retain, nonatomic) NSMutableDictionary *controllers; // @synthesize controllers=_controllers;
@property(retain, nonatomic) UIScrollView *contentScroll; // @synthesize contentScroll=_contentScroll;
@property(retain, nonatomic) HMSegmentedControl *segmentControl; // @synthesize segmentControl=_segmentControl;
- (void).cxx_destruct;
- (void)_loadControllerDataForKey:(id)arg1;
- (id)_getControllerForKey:(id)arg1;
- (void)scrollViewDidEndDecelerating:(id)arg1;
- (void)segmentedControlChangedValue:(id)arg1;
- (void)viewDidLoad;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

